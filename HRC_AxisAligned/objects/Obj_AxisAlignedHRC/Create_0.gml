...
render_emissv = surface_build(render_extent, render_extent, surface_rgba8unorm, render_surflist);
render_absorp = surface_build(render_extent, render_extent, surface_rgba8unorm, render_surflist);
render_fluences = surface_build(render_extent, render_extent, surface_rgba8unorm, render_surflist);

for(var i = 0; i < render_count; i++) {
	merge_radiance[i] = surface_build(render_extent, render_extent, surface_rgba8unorm, render_surflist);
}

for(var i = 0; i < 4; i++)
	render_frustums[i] = surface_build(render_extent, render_extent, surface_rgba8unorm, render_surflist);

merginghrc_u_Shader         = Shd_MergingAAHRC;
...
merginghrc_u_CascadeScalar  = uniform(merginghrc_u_Shader, "cascade_scalar");
