...
gpu_set_tex_filter(false);
	...
	for(var j = 0; j < 4; j++) {
		// Clear C0 (highest cascade recklessly reads from C0).
		surface_set_target(surface_source(merge_radiance[0]));
		...
		for(var i = (render_count - 1); i >= 0; i--) {
			...
			shader_float(merginghrc_u_CascadeScalar, scalar);
...
//draw_surface_ext(surface_source(merge_radiance[render_index]), 0, 0, xx, xx, 0, c_white, 1.0);
...
draw_text(5,  25, "Cascade Index:   " + string(render_index));